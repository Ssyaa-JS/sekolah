...
// Mengambil data dari tabel peminjaman diurutkan dari yang terbaru
...
<head>
    ...
    <title>Transaksi Pinjam - SekolahApp</title>
...
<body class="p-6 md:p-10">
    <nav class="max-w-[98%] mx-auto mb-8 flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <ul class="flex gap-2">
            ...
            <li><a href="index_pmn.php" class="px-4 py-2 rounded-xl text-sm font-bold bg-indigo-50 text-indigo-600 border border-indigo-100">Peminjaman</a></li>
    ...
    <div class="max-w-[98%] mx-auto">
        <div class="mb-6 flex justify-between items-center px-2">
            ...
            <a href="add_pmn.php" class="bg-slate-900 text-white px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-indigo-600 transition shadow-lg flex items-center gap-2">
        ...
        <div class="card-3d rounded-[2rem] overflow-hidden">
                    ...
                    <tbody>
                        <?php while ($user_data = mysqli_fetch_array($result)) { ?>
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="font-bold text-indigo-600 italic">#<?php echo $user_data['id_peminjaman']; ?></td>
                                <td class="font-bold text-slate-700">ALT-<?php echo $user_data['id_alat']; ?></td>
                                <td class="text-slate-700">
                                    <span class="bg-slate-100 px-2 py-1 rounded-md font-semibold text-xs">PM-<?php echo $user_data['id_peminjam']; ?></span>
                                </td>
                                <td class="text-slate-500 text-[11px]">
                                    <?php echo date('d M Y', strtotime($user_data['tanggal_pinjam'])); ?>
                                </td>
                                <td class="text-rose-500 text-[11px] font-semibold">
                                    <?php echo date('d M Y', strtotime($user_data['tanggal_kontrak'])); ?>
                                </td>
                                <td class="text-center">
                                    <span class="bg-indigo-50 text-indigo-600 px-3 py-1 rounded-lg font-bold text-xs border border-indigo-100">
                                        <?php echo $user_data['stok']; ?>
                                    </span>
                                </td>
                                <td class="text-center">
                                    <div class="flex justify-center gap-2">
                                        <a href="edit_pmn.php?id_peminjaman=<?php echo $user_data['id_peminjaman']; ?>"
                                        ...
                                        <a href="javascript:void(0)"
                                            onclick="confirmDelete(<?php echo $user_data['id_peminjaman']; ?>)"
            ...
            <div class="bg-slate-50 p-4 border-t border-slate-100 text-center">
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Total Transaksi: <?php echo mysqli_num_rows($result); ?></p>
            </div>
        </div>
    </div>
